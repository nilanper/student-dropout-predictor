...
def show_guide_image(filename: str, caption: str):
    ...
    if image_path.exists():
        ...
        img = img.resize((max(1, int(img.width * 0.5)), max(1, int(img.height * 0.5))))
...
with guide_tab:
    ...
    with st.expander("1️⃣ Overview of the System", expanded=True):
        st.markdown("""
This system can be used by individual educational institutions to:

- train their own machine learning models using historical student data
- make predictions for current students using current student data
- understand why a student is predicted at risk using SHAP explanations

The system supports dataframes of any shape and supports CSV, text, and Excel uploads.
For Excel files with multiple sheets, select the correct sheet before continuing.
For CSV/TXT files, the app automatically detects and displays the delimiter.

For meaningful predictions, the prediction file’s columns must exactly match the model training file.
""")

    with st.expander("2️⃣ About the Two Main Tabs"):
        st.markdown("""
**Tab 1 – Train Institution Model**
- upload historical data
- configure training settings
- train a model
- review model performance and overall SHAP explanations
...
""")
    ...
    with st.expander("3️⃣ Step-by-Step: Training a Model"):
        ...
        st.markdown("Use the **Test Split Proportion** slider to decide how much data is used for testing versus training.")

        st.markdown("### Step 5: Choose Model")
        ...
        st.markdown("Click **🚀 Train Model** to begin model training.")

        st.markdown("### Step 8: View Results")
        st.markdown("Review the model performance results and overall SHAP outputs.")

        show_guide_image("Tab1_after_model_training.png", "Training Results & SHAP Summary")
    ...
    with st.expander("4️⃣ Step-by-Step: Making Predictions"):
        ...
        st.markdown("The app automatically checks whether the uploaded student file is compatible with the trained model. If there is an incompatibility, correct the columns and re-upload the file.")
        ...
        st.markdown("Review the generated prediction table, including student identifiers, probability, and prediction label.")
    ...
    with st.expander("5️⃣ SHAP Explanation & Interpretation"):
        ...
        st.markdown("""
This plot explains why a specific student was predicted as Dropout or No Dropout.
...
""")
    ...
    with st.expander("6️⃣ Validation, Errors, and Re-uploading Files"):
        st.markdown("""
